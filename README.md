...
Before using ChatGPT as the UI regression environment, run
`npm run test:webgpt-local` for the sandbox/AppBridge/browser gate, or
`npm run test:webgpt-local:ui` to open the visual local compatibility lab.
See [Web GPT Local Compatibility Testing](docs/webgpt-local-testing.md) for the
coverage matrix and the remaining real-Web-GPT smoke boundary.
