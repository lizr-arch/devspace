...
async function run(): Promise<void> {
  ...
  check(
    "a. create_handoff_from_webgpt OK",
    r1.status === "OK",
    r1.status as string,
  );
  ...
  check(
    "d. run_orchestrator_step COMPLETED",
    r4.status === "COMPLETED",
    r4.status as string,
  );
  ...
  check(
    "e. read_run_artifacts has artifacts",
    typeof r5.artifacts === "object",
    JSON.stringify(Object.keys(r5.artifacts as object)),
  );

  console.log("  Step f: read_current_task");
  const r6 = (await callTool("read_current_task", {})) as R;
  check(
    "f. read_current_task has content",
    typeof r6.content === "string" && (r6.content as string).length > 0,
  );

  console.log("  Step g: read_delegate_timeline");
  const r7 = (await callTool("read_delegate_timeline", {})) as R;
  check(
    "g. timeline has entries",
    Array.isArray(r7.entries) && (r7.entries as unknown[]).length > 0,
  );

  console.log("  Step h: get_delegate_status");
  const r8 = (await callTool("get_delegate_status", {})) as R;
  check(
    "h. status has mode field",
    typeof r8.mode === "string" || r8.mode === null,
  );

  console.log("  Step i: stop_delegate_run");
  const r9 = (await callTool("stop_delegate_run", {
    run_token: runToken,
  })) as R;
  check(
    "i. stop_delegate_run STOPPED",
    r9.status === "STOPPED",
    r9.status as string,
  );
  ...
  if (existsSync(auditPath)) {
    ...
    check(
      "k. audit covers 8+ tools",
      tools.size >= 8,
      `found ${tools.size} tools: ${[...tools].sort().join(", ")}`,
    );
  }
