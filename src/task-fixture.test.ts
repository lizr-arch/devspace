...
async function test7_manifestModificationStale() {
  ...
  assert.ok(!checkManifestIntegrity(dir, approved), "should be stale after modification");
  await rm(dir, { recursive: true, force: true });
...
async function test8_paramsInvalidFail() {
  ...
  assert.ok(r1.errors.length > 0, "should have error for missing required param");
...
async function test9_stdoutStderrExitCode() {
  ...
  assert.ok(result.stdout.includes("hello"), `stdout should contain 'hello': ${result.stdout}`);
  assert.ok(result.stderr.includes("world"), `stderr should contain 'world': ${result.stderr}`);
  await rm(dir, { recursive: true, force: true });
...
async function test13_structuredErrorsHaveRequiredFields() {
  ...
  console.log("PASS: test13 - structured errors have code, manifestPath, recoverable, message");
}
...
`);
  ...
  assert.ok(command.every((a) => !a.includes("$(whoami)")), "shell injection not in command");
  await rm(dir, { recursive: true, force: true });
  console.log("PASS: test19 - shell metacharacters in params not injected into argv");
}
...
`;
  ...
  await rm(dir, { recursive: true, force: true });
  console.log("PASS: test20 - unsupported executor returns TASK_EXECUTOR_UNSUPPORTED");
}
