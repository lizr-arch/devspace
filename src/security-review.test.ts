...
import { TaskRunner, resolveWorkspacePythonEnvironment } from "./task-runner.js";
...
function test_av1_start_job_schema_no_secret_params(): void {
  ...
  try {
    ...
    const { command, errors } = validateAndSubstitute(
      task,
      { secret: "injected", env: "bad", credential: "bad2", environment: "bad3" },
      dir,
      [],
    );
    ...
    console.log("PASS: AV1 - start_job/project_task params don't accept secret/env/credential/environment");
  } finally {
    safeCleanup(dir);
  }
}
...
`,
    ...
    assert.equal(command.length, 3, "param injection does NOT add argv elements");
...
function test_av4_manifest_mod_approval_reuse(): void {
  ...
  try {
    ...
    assert.ok(checkManifestIntegrity(dir, approved), "original should be intact");
    assert.ok(isTaskApproved("echo-secret", approved));

    // Modify manifest: add an exfiltration task
    const modifiedManifest = SECRET_TASK_MANIFEST + `
  exfiltrate:
    mode: run
...
function test_av5_source_change_no_manifest_change(): void { // sync — no await
...
`;
    ...
    console.log("FINDING: AV5 - Source code change does NOT invalidate approval");
    console.log("         This is an inherent design tradeoff — manifest SHA only");
    console.log("         covers the task definition, not the project source code.");
    console.log("         Mitigation: capability fingerprint includes environment +");
...
`,
    ...
    if (result.mode === "run" && result.status === "succeeded") {
      ...
      if (escaped) {
        ...
        console.log("         Tasks have UNRESTRICTED filesystem access via spawn().");
        console.log("         No chroot/jail/namespace isolation.");
        rmSync(outsidePath, { force: true });
      } else {
        console.log("PASS: AV7 - File outside workspace was NOT created (unexpected)");
      }
    } else {
      console.log(
...
async function test_av8_stdout_stderr_secret_leak(): Promise<void> {
  ...
  try {
    ...
    if (result.mode === "run") {
      ...
      console.log("PASS: AV8 - Secrets redacted from stdout/stderr in run mode");
    }
  } finally {
    safeCleanup(dir);
  }
...
function test_av9_concurrent_job_secret_leak(): void {
  ...
  console.log("PASS: AV9 - Concurrent jobs cannot leak secrets via shared state");
}
...
`;
    ...
    for (const payload of shellPayloads) {
      ...
      assert.equal(errors.length, 0, `param '${payload}' should be accepted as string`);
    ...
    console.log("PASS: AV12 - Shell metacharacters NOT injected into command argv");
  } finally {
    safeCleanup(dir);
  }
}
...
`,
    ...
    assert.equal(afterGlobalEnv, beforeGlobalEnv, "secret should NOT leak to global process.env");
...
`;
    ...
    try {
      ...
      assert.ok(
        te.message.includes("secret_ref") || te.code === "TASK_MANIFEST_SCHEMA_ERROR"
      );
    }
...
`;
    ...
    console.log("PASS: AV15 - Secret type coercion blocked by schema validation");
  } finally {
    safeCleanup(dir);
  }
}
...
function test_av16_redaction_bypass_unicode(): void {
  ...
  const result1 = redactSecrets(
    `Token: sk_live_evilhacker\u200Btoken`,
    [zombieSecret],
  );
  assert.ok(
  ...
  const result3 = redactSecrets(`before\nline1\nline2\nafter`, [multilineSecret]);
  assert.ok(result3.includes("[REDACTED_SECRET]"));

  console.log("PASS: AV16 - Redaction handles Unicode/encoding tricks correctly");
}
...
function test_av17_approval_bypass_no_manifest(): void {
  ...
  try {
    ...
    assert.ok(!checkManifestIntegrity(dir, approved), "null vs non-null should not match");
...
async function test_av18_task_id_approval_spoofing(): Promise<void> {
  ...
  try {
    ...
    console.log("PASS: AV18 - Task ID spoofing in approval blocked by manifest validation");
  } finally {
    safeCleanup(dir);
  }
}
...
`;
    ...
    try {
      ...
      console.log("  Path traversal in secret_ref is treated as literal key (safe)");
    } catch (e) {
      console.log(`  Execution blocked: ${e}`);
    }

    console.log("PASS: AV19 - Secret ref path traversal treated as literal key");
  } finally {
    safeCleanup(dir);
  }
}
...
function test_av20_path_param_outside_roots(): void {
  ...
  try {
    ...
    const outsidePath = isWin ? "C:\\Windows\\System32\\drivers\\etc\\hosts" : "/etc/passwd";
...
`;
    ...
    try {
      ...
      console.log(`  Valid env name accepted: ${result.mode === "run" ? result.status : "session"}`);
    } catch (e) {
...
`;
    ...
    try {
      ...
      const msg = String(e);
      assert.ok(
        msg.includes("Invalid target_env") || msg.includes("TASK_MANIFEST_SCHEMA_ERROR"),
...
async function main(): Promise<void> {
  ...
  const tests: Array<{ name: string; fn: () => void | Promise<void> }> = [
    { name: "AV1 - start_job schema no secret params", fn: test_av1_start_job_schema_no_secret_params },
    { name: "AV2 - Param argv injection blocked", fn: test_av2_param_argv_injection },
    { name: "AV3 - target_env override blocked", fn: test_av3_target_env_override },
    { name: "AV4 - Manifest mod + old approval reuse blocked", fn: test_av4_manifest_mod_approval_reuse },
    { name: "AV5 - Source change without manifest change (FINDING)", fn: test_av5_source_change_no_manifest_change },
    { name: "AV6 - Dirty state execution (FINDING)", fn: test_av6_dirty_state_execution },
    { name: "AV7 - Artifact root escape (FINDING)", fn: test_av7_artifact_root_escape },
    { name: "AV8 - stdout/stderr secret redaction", fn: test_av8_stdout_stderr_secret_leak },
    { name: "AV9 - Concurrent job secret leak blocked", fn: test_av9_concurrent_job_secret_leak },
    { name: "AV10 - Git filter bypass checked", fn: test_av10_git_filter_bypass },
    { name: "AV11 - Nested .gitattributes bypass blocked", fn: test_av11_nested_gitattributes_bypass },
    { name: "AV12 - Shell metacharacter injection blocked", fn: test_av12_shell_metacharacter_injection },
    { name: "AV13 - Runtime override bypass blocked", fn: test_av13_runtime_override_bypass },
    { name: "AV14 - No secret in global process.env", fn: test_av14_secret_not_in_global_env },
    { name: "AV15 - Secret type coercion blocked", fn: test_av15_secret_type_coercion },
    { name: "AV16 - Redaction bypass (unicode) blocked", fn: test_av16_redaction_bypass_unicode },
    { name: "AV17 - Approval bypass no manifest blocked", fn: test_av17_approval_bypass_no_manifest },
    { name: "AV18 - Task ID spoofing blocked", fn: test_av18_task_id_approval_spoofing },
    { name: "AV19 - Secret ref path traversal harmless", fn: test_av19_secret_ref_path_traversal },
    { name: "AV20 - Path param outside roots blocked", fn: test_av20_path_param_outside_roots },
    { name: "AV21 - target_env validation", fn: test_av21_target_env_validation },
    { name: "AV22 - Concurrent session secret leak blocked", fn: test_av22_concurrent_session_secret_leak },
  ];
  ...
  console.log("  AV1  - start_job schema has no secret/env/credential/environment params");
  console.log("  AV2  - Param values cannot inject new argv elements");
  console.log("  AV3  - target_env is fixed in manifest, cannot be overridden via params");
  console.log("  AV4  - Manifest SHA change invalidates old approval");
  console.log("  AV8  - Secrets redacted from stdout/stderr in both run and session mode");
  console.log("  AV9  - Secrets go to per-process childEnv, not global process.env");
  console.log("  AV10 - assertNoEffectiveGitFilters detects configured filter drivers");
  console.log("  AV11 - Nested .gitattributes correctly resolved by git check-attr");
  console.log("  AV12 - spawn() without shell prevents metacharacter interpretation");
  ...
  console.log("  AV15 - Schema validation rejects non-string secret_ref/target_env");
  ...
  console.log("  AV18 - Task ID spoofing blocked by manifest validation on execution");
  console.log("  AV19 - Secret ref treated as literal key, no filesystem access");
  console.log("  AV20 - Path param validation rejects paths outside allowed roots");
  ...
  console.log("  AV5  - Source code changes don't invalidate manifest approval");
  console.log("         Risk: Modified Python source can exfiltrate secrets on re-execution");
  console.log("         Mitigation: Capability fingerprint includes environment + lock SHA");
  console.log("         Mitigation: Git hooks/pre-commit checks should gate execution");
  console.log("  AV6  - No git cleanliness check before secret-bearing task execution");
  console.log("         Risk: Staged/uncommitted changes could contain malicious code");
  console.log("         Mitigation: Server layer should check git status before execution");
  ...
  console.log("         Mitigation: OS-level sandboxing (containers/chroot) or seccomp");
  console.log("");
  ...
  if (failed > 0) {
    ...
    console.log("  Design tradeoffs (AV5, AV6, AV7) are documented and have mitigations.");
